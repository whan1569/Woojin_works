...
void _CYCLIC MHC_InterfaceCyclic(void)
{
	...
	if(startupfuntion_flag == 1 ){
		startupfuntion(Rx_buffer);
	}
	if(setdata_change_flag == 1 && startupfuntion_flag == 0){
		setdata_change();
	}
	if(REC_DATA.MHC_CTRL.EnableCommunication){  //KBH 151210 수정 //170313 YJM REC데이터로 변경
		MHC_Alarm_Process();	
	}
	if(REC_DATA.MHC_CTRL.EnableCommunication && MMI_DATA.Mode == MODE_AUTO && MMI_DATA.Mode == MODE_SEMI_AUTO){  //KBH 151210 수정 //160309_leejs //170313 REC데이터로 변경
		MHC_Alarm_Process();
	}
	...
	switch(Receiver.step)
	{
		...
		case FRMR_COPY_RD_DATA:
			...
			for(q=0;q<Receiver.buffer_length;q++){
				if((Rx_buffer[rxthbuf_count -1]==0x03)&&(rxthbuf_count>1)){
			...
			if(hot_startupflag == 1)
			{
				...
				if(MA_FIX.OPT.MHC_Page_Enable == 1){
					if(REC_DATA.MHC_CTRL.EnableCommunication == 1){	//170313 YJM REC데이터로 변경
						...
						LRCresult = LRCcal(startup_code,7);
						startup_code[6] = LRCresult;
						send_dvf_tx_data(startup_code,7);
						//send_dvf_tx_data(mk_send,5);  //화인전자와 한영 넉스 구분하기 위해 보냄  autoconfig //KBH 151204 호환성 문제로 주석 처리  
					}
				}else if(MA_FIX.OPT.MHC_Page_Enable == 2){
